...
export function uninit() {
	Me = null;
	filesDir = null;
	shortcutssFilePath = null;
	longestEntry = { app: '', type: '', shortcut: '', description: '' };
}

// RowModel GObject class
export const RowModel = GObject.registerClass(
	{
		Properties: {
...
export async function loadListStore(listStore, file = Gio.File.new_for_path(shortcutssFilePath)) {
	const jsonData = await readShortcutsJson(file);
...
export async function readShortcutsJson(file = Gio.File.new_for_path(shortcutssFilePath)) {
