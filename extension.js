...
const FullscreenWidget = GObject.registerClass(
	class FullscreenWidget extends St.Widget {
		_init() {
			...
			if (extension.parentBox === null) {
				extension.parentBox = new St.BoxLayout({ vertical: true, x_expand: true, });
				extension.resetlayout();
			} else if (!extension.firstLaunch && !extension.parentBox.isError) {
				try {
					extension.parentBox.resetSearchEntry();
				} catch (e) {
					log(e.message);
				}
			}
			this.scrollView.set_child(extension.parentBox);
			this.add_child(this.scrollView);
			if (extension.firstLaunch) extension.buildLayout(monitor.width);
			this.connect('button-press-event', () => {
				this.scrollView.set_child(null);
				extension.fullscreenOverlay.destroy();
				extension.fullscreenOverlay = null;
			...
			this.grab_key_focus();
			if (extension.firstLaunch) {
				this.connect('notify::allocation', () => {
					extension.updateShortcutsByRanks(extension.indicesToUpdate);
					const columns = extension.createColumns();
					extension.createComponents(columns);
					extension.firstLaunch = false;
					extension.freeMemory();
				});
			}
		}
	}
...
export default class ShortcutsPopupExtension extends Extension {
	...
	enable() {
		...
		this.settings = this.getSettings(SETTINGS_SCHEMA);
		...
		// Set module-level extension reference for use by other classes
		extension = this;
		...
		this.statusIcon = new StatusIcon(this.isIconHidden, this.filesDir);
	...
	disable() {
		...
		// Clear module-level extension reference
		extension = null;
	}

	openLayoutShortcuts() {
		if (!this.fullscreenOverlay) {
			this.fullscreenOverlay = new FullscreenWidget();
	...
	resetlayout() {
		this.firstLaunch = true;
		this.resultReadJsonFile = Common.readShortcutsJson();
